...
class MultiCameraDiffusionInference:
    ...
    def __init__(self, model_path: str, config_path: str, device: str = 'cuda'):
        ...
        self.action_mean = np.array(self.config.get('action_mean', [0.0] * action_dim))
        self.action_std = np.array(self.config.get('action_std', [1.0] * action_dim))
        ...
        print(f"  动作维度: {self.config.get('action_dim', 8)} (7关节+1夹爪)")
    ...
    def predict_action(self) -> np.ndarray:
        ...
        with torch.no_grad():
            actions = self.model.sample(
                images, 
                states, 
                num_inference_steps=self.config.get('num_inference_steps', 100)  # 增加采样步数
            )
...
def main():
    ...
    load_path = '/home/alien/simulation/robot-colosseum/diffusion_policy/my_model/CloseBox'
    ...
    for episode in range(args.episodes):
        ...
        for step in range(args.max_steps):
            ...
            # 显示进度
            if step % 20 == 0:
                joint_delta_norm = np.linalg.norm(action[:7])
