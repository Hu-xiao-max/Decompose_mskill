...
class SimpleTrainer:
    ...
    def compute_loss(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if len(images_list) == 0:
            ...
            try:
                ...
                # 训练时的随机相机dropout以增强鲁棒性
                cam_drop_prob = float(self.config.get('camera_dropout_prob', 0.0))
                if self.model.training and cam_drop_prob > 0.0:
                    B, NC = images.shape[0], images.shape[1]
                    drop_mask = (torch.rand(B, NC, device=self.device) < cam_drop_prob)
                    # 确保每个样本至少保留一个相机
                    all_dropped = drop_mask.sum(dim=1) == NC
                    if all_dropped.any():
                        idx = torch.randint(0, NC, (int(all_dropped.sum().item()),), device=self.device)
                        drop_mask[all_dropped, :] = True
                        drop_mask[all_dropped, idx] = False
                    # 将被drop相机置零
                    drop_mask_exp = drop_mask.view(B, NC, 1, 1, 1, 1)
                    images = images.masked_fill(drop_mask_exp, 0.0)
        ...
        # Classifier-Free Guidance: 训练时对一部分样本丢弃条件（置零）
        cfg_drop_prob = float(self.config.get('cfg_dropout_prob', 0.0))
        if cfg_drop_prob > 0.0:
            with torch.no_grad():
                drop_mask = (torch.rand((batch_size,), device=self.device) < cfg_drop_prob)
            if drop_mask.any():
                images[drop_mask] = 0.0
                robot_states[drop_mask] = 0.0
        
        # 前向传播
        try:
            pred_type = getattr(self.model, 'prediction_type', 'epsilon')
            if self.use_amp:
                with autocast():
                    pred = self.model(noisy_actions, timesteps, images, robot_states)
                    if pred_type == 'v_prediction':
                        alpha_t = self.model.sqrt_alphas_cumprod[timesteps].view(-1, 1, 1)
                        sigma_t = self.model.sqrt_one_minus_alphas_cumprod[timesteps].view(-1, 1, 1)
                        target = alpha_t * noise - sigma_t * actions
                    else:
                        target = noise
                    # p2/SNR加权
                    with torch.no_grad():
                        snr = (self.model.alphas_cumprod[timesteps] / (1 - self.model.alphas_cumprod[timesteps])).view(-1, 1, 1)
                        p2_k = float(self.config.get('p2_weight_k', 1.0))
                        p2_gamma = float(self.config.get('p2_weight_gamma', 0.5))
                        weights = (p2_k + snr).pow(-p2_gamma)
                    # 选择损失类型
                    loss_type = self.config.get('loss_type', 'mse')
                    if loss_type == 'huber':
                        delta = float(self.config.get('huber_delta', 1.0))
                        base_loss = F.smooth_l1_loss(pred, target, reduction='none', beta=delta)
                    else:
                        base_loss = (pred - target) ** 2
                    mse_loss = (weights * base_loss).mean()
            else:
                pred = self.model(noisy_actions, timesteps, images, robot_states)
                if pred_type == 'v_prediction':
                    alpha_t = self.model.sqrt_alphas_cumprod[timesteps].view(-1, 1, 1)
        ...
        # 计算平滑项：在预测的x0上进行时间平滑正则
        with torch.no_grad():
            alpha_t = self.model.sqrt_alphas_cumprod[timesteps].view(-1, 1, 1)
            sigma_t = self.model.sqrt_one_minus_alphas_cumprod[timesteps].view(-1, 1, 1)
        if pred_type == 'v_prediction':
            x0_pred = alpha_t * noisy_actions - sigma_t * pred
        else:
            x0_pred = (noisy_actions - sigma_t * pred) / (alpha_t + 1e-8)
        # 一阶与二阶差分
        smooth_w = float(self.config.get('smooth_loss_weight', 0.0))
        accel_w = float(self.config.get('accel_loss_weight', 0.0))
        smooth_loss = torch.tensor(0.0, device=self.device)
        accel_loss = torch.tensor(0.0, device=self.device)
        if smooth_w > 0.0 and x0_pred.shape[1] > 1:
            diff1 = x0_pred[:, 1:, :] - x0_pred[:, :-1, :]
            smooth_loss = diff1.abs().mean()
        if accel_w > 0.0 and x0_pred.shape[1] > 2:
            diff2 = x0_pred[:, 2:, :] - 2 * x0_pred[:, 1:-1, :] + x0_pred[:, :-2, :]
            accel_loss = diff2.abs().mean()

        total_loss = mse_loss + smooth_w * smooth_loss + accel_w * accel_loss
        losses = {
            'mse_loss': mse_loss,
            'smooth_loss': smooth_loss.detach(),
            'accel_loss': accel_loss.detach(),
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        grad_accum = max(int(self.config.get('grad_accum_steps', 1)), 1)
        for batch_idx, batch in enumerate(pbar):
            ...
            try:
                ...
                # 反向传播
                if self.use_amp:
                    (self.scaler.scale(total_loss) / grad_accum).backward()
                    # 更新与梯度裁剪在累计步时机执行
...
def create_optimized_config() -> Dict:
    ...
    return {
        ...
        'augment_images': True,
        'camera_dropout_prob': 0.15,  # 训练时随机丢弃部分相机
        ...
        'fusion_method': 'attention',  # 可选: attention / gated_avg
        'prediction_type': 'v_prediction',  # 扩散预测类型
        'clip_range': [-2.0, 2.0],  # 训练与推理一致的动作范围
        'cfg_dropout_prob': 0.2,           # CFG训练丢弃概率（增强鲁棒性）
        'p2_weight_k': 1.0,                # p2/SNR加权参数k
        'p2_weight_gamma': 0.5,            # p2/SNR加权参数gamma
        'loss_type': 'mse',                # 可选: mse / huber
        'huber_delta': 1.0,                # Huber阈值
        'smooth_loss_weight': 0.05,        # 一阶平滑正则权重
        'accel_loss_weight': 0.01,         # 二阶平滑正则权重
        ...
        'grad_accum_steps': 1,  # 梯度累计步数（可增大以提升有效batch）
        'patience': 80,  # 增加耐心值
